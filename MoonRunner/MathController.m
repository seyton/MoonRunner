...
+ (NSArray *)colorSegmentsForLocations:(NSArray *)locations {
    ...
    for (int i =1; i < locations.count; i++) {
        ...
        CLLocation *firstLocCL = [[CLLocation alloc] initWithLatitude:firstLoc.latitude.doubleValue
                                                            longitude:firstLoc.longitude.doubleValue];
        
        CLLocation *secondLocCL = [[CLLocation alloc] initWithLatitude:secondLoc.latitude.doubleValue
    ...
    for (int i= 1; i < locations.count; i++) {
        ...
        //mix red with yellow for between segments.
        if (speed.doubleValue < meanSpeed) {
            ...
            CGFloat blue = y_blue + ratio * (g_blue - r_blue);
            
            color = [UIColor colorWithRed:red green:green blue:blue alpha:1.0];
        }
        
        MulticolorPolylineSegment *segmen = [MulticolorPolylineSegment polylineWithCoordinates:coords count:2];
        segmen.color = color;
        
        [colorSegments addObject:segmen];
